...
class lyricsmint:
    ...
    def do_search(self):
        ...
        self.url=self.search_url+self.title[0]
        return self.url
    ...
    def get_link(self,searchpage):
        ...
        self.html_search=searchpage
        self.search = BeautifulSoup(self.html_search, 'html.parser')
        post=self.search.find_all("div", class_="post-title")
        for block in post:
            self.linklist=block.find_all("a")
            if len(self.linklist)>0:
                return self.linklist[0].get('href')
    ...
    def get_lyrics(self,lyricspage):
        ...
        self.html_lyrics=lyricspage
        self.soup = BeautifulSoup(self.html_lyrics, 'html.parser')
        self.songlyric=self.soup.find_all("div", id="lyric")
        self.lyrics= self.songlyric[0].find_all("p")
